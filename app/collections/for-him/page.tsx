...
function EssentialArchiveSection() {
  const [products, setProducts] = useState<EssentialProduct[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    const SUPABASE_URL = 'https://uvgtgeauhjbdatrmmaob.supabase.co'
    const ANON_KEY = 'sb_publishable_VuaEqan3EBtGHbpTI0KdJg_OimrHkqM'
    const COLLECTION_ID = '4a606e76-051b-40d8-b218-9e587a052120'

    fetch(
      `${SUPABASE_URL}/rest/v1/products?collection_id=eq.${COLLECTION_ID}&is_active=eq.true&order=is_featured.desc,name.asc`,
      {
        headers: {
