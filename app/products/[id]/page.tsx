...
function ProductJsonLd({ product }: { product: Product }) {
  ...
  let story: { productCode?: string; faqs?: { question: string; answer: string }[]; verification?: { blockchain?: boolean } } = {}
  try {
    story = typeof product.story === 'string' ? JSON.parse(product.story) : (product.story as any) || {}
  } catch {
    story = {}
  }
  const productCode = story.productCode || product.slug
  const faqs = Array.isArray(story.faqs) ? story.faqs : []
    ...
    '@graph': [
      {
        ...
        ...(isSovereign && story.verification?.blockchain !== false && {
          additionalProperty: [
