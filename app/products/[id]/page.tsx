...
function ScentPyramid({ pyramid }: { pyramid: ProductDetails['scentPyramid'] }) {
  ...
  const layers = [
    { label: 'TOP', sublabel: 'Opening', value: pyramid.top, delay: 0 },
    { label: 'HEART', sublabel: 'Core', value: pyramid.heart, delay: 0.1 },
    { label: 'BASE', sublabel: 'Anchor', value: pyramid.base, delay: 0.2 },
  ]
  return (
    <div className="space-y-1">
      {layers.map(layer => (
        <motion.div key={layer.label} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
          ...
          <div className="flex-1 bg-gradient-to-r from-[#c9a054]/20 to-[#c9a054]/5 border-l-2 border-[#c9a054] px-5 py-4">
...
const SOVEREIGN_SLUGS = ['shamims-bloom', 'queen-of-taif', 'her-legacy-vault', 'shamim-bloom-the-sovereign-grace', 'shamim-bloom-sovereign-grace']
...
export default function ProductDetailPage({ params }: { params: { id: string } }) {
  ...
  const handleWeb3Success = useCallback(async (txHash: string, coin: CoinType) => {
    try {
      ...
      setOrderError(e?.message || 'Order save failed — your crypto payment went through. Contact us with your TX hash.')
    }
  ...
  async function handlePlaceOrder() {
    ...
    if (payMethod === 'pkr_manual' && !txId && !proofFile) { setOrderError('Please provide Transaction ID or upload payment screenshot.'); return }
  ...
  if (!product) return (
    <div className="min-h-screen bg-[#050505] pt-20 flex flex-col items-center justify-center gap-8">
      <p className="font-[Cormorant_Garamond] text-4xl font-light text-zinc-700">Creation Not Found</p>
  ...
  // Route all sovereign / special products to the cinematic luxury page
  if (SOVEREIGN_SLUGS.includes(product.slug)) return <SovereignProductPage product={product} />
  ...
  if (orderResult) return (
    <div className="min-h-screen bg-[#050505] pt-20 flex items-center justify-center px-6">
      <motion.div initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 0.8 }} className="text-center max-w-lg w-full">
        <p className="font-[Cormorant_Garamond] text-7xl text-[#c9a054] mb-8">◆</p>
        <h2 className="font-[Cormorant_Garamond] text-4xl font-light tracking-[0.2em] uppercase text-zinc-100 mb-3">Order Placed</h2>
        <p className="text-[8px] tracking-[0.45em] uppercase text-[#c9a054] mb-10">House of Shamim Forever</p>
        <div className="border border-[#1a1a1a] divide-y divide-[#111] mb-8 text-left">
          {[['Order Reference', orderResult.order_ref],['Tracking ID', orderResult.tracking_ref],['Status', orderResult.status?.replace(/_/g,' ')]].map(([lbl, val]) => (
            <div key={lbl} className="flex justify-between items-center px-6 py-4">
              <p className="text-[7px] tracking-[0.4em] uppercase text-zinc-600">{lbl}</p>
              <div className="flex items-center gap-2">
                <p className="text-zinc-100 font-mono text-sm font-light">{val}</p>
                {lbl !== 'Status' && <CopyButton text={val ?? ''} />}
              </div>
            </div>
          ))}
        </div>
        <div className="flex gap-3 justify-center">
          <Link href={orderResult.track_url} className="flex items-center gap-2 px-6 py-3 border border-[#c9a054]/40 text-[8px] tracking-[0.4em] uppercase text-[#c9a054] hover:bg-[#c9a054]/10 transition-all"><ExternalLink size={11} /> Track Order</Link>
          <Link href="/shop" className="px-6 py-3 border border-zinc-800 text-[8px] tracking-[0.4em] uppercase text-zinc-500 hover:text-zinc-300 transition-all">Continue</Link>
        </div>
      </motion.div>
    </div>
  )
      ...
      <div className="max-w-[1600px] mx-auto px-6 md:px-12 lg:px-20 py-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
              ...
              <AnimatePresence mode="wait">
                {images.length > 0 ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center">
                    <p className="font-[Cormorant_Garamond] text-9xl text-[#c9a054]/10">SF</p>
                  </div>
                )}
              </AnimatePresence>
              {images.length > 1 && (
                <>
                  <button onClick={() => setActiveImage(i => Math.max(0, i - 1))} className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center bg-[#050505]/80 text-zinc-400 hover:text-[#c9a054] transition-colors"><ChevronLeft size={16} /></button>
                  <button onClick={() => setActiveImage(i => Math.min(images.length - 1, i + 1))} className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center bg-[#050505]/80 text-zinc-400 hover:text-[#c9a054] transition-colors"><ChevronRight size={16} /></button>
                </>
              )}
            </div>
            {images.length > 1 && (
          ...
          <div>
            ...
            <h1 className="font-[Cormorant_Garamond] font-light text-4xl md:text-5xl tracking-[0.1em] text-zinc-100 leading-tight mb-4">{product.name}</h1>
            {details?.tagline && <p className="font-[Cormorant_Garamond] italic text-zinc-500 text-lg mb-8">{details.tagline}</p>}
            <div className="flex items-baseline gap-4 mb-10 pb-10 border-b border-[#111]">
              <p className="font-[Cormorant_Garamond] text-3xl font-light text-zinc-100">{formatPKR(finalPkr)}</p>
            ...
            <AnimatePresence mode="wait">
              {activeTab === 'story' && (
                <motion.div key="story" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-8 mb-10">
                  ...
                  {details?.olfactory && <p className="font-[Cormorant_Garamond] italic text-zinc-600 text-base leading-[2] border-l-2 border-[#c9a054]/20 pl-5">{details.olfactory}</p>}
            ...
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                <p className="font-[Cormorant_Garamond] text-2xl font-light text-zinc-100">{formatPKR(finalPkr)}</p>
              ...
              <div className="flex border border-[#1a1a1a] mb-5">
                {(['crypto', 'pkr_manual', 'cod'] as PayMethod[]).map(m => (
                  <button key={m} onClick={() => setPayMethod(m)}
                    className={`flex-1 py-3 text-[8px] tracking-[0.22em] uppercase transition-all duration-300 border-b-2 ${payMethod === m ? 'bg-[#c9a054]/8 text-[#c9a054] border-b-[#c9a054]' : 'text-zinc-600 border-b-transparent hover:text-zinc-400'}`}>
                    {m === 'crypto' ? '◆ Crypto' : m === 'pkr_manual' ? 'PKR Bank' : 'COD'}
                  </button>
                ))}
              </div>

              {payMethod === 'crypto' && <Web3PaySection priceUsd={product.price_usd * quantity} onSuccess={handleWeb3Success} />}
