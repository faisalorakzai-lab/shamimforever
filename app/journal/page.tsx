...
export default function JournalPage() {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden" style={{ background: '#070a1a', color: '#e8e8e8', fontFamily: "'Inter', sans-serif" }}>
      ...
      {/* ── TOP BAR ── */}
      <div className="hidden md:flex items-center justify-between px-6 py-3 border-b" style={{ borderColor: '#1a1d2e', fontSize: '10px', letterSpacing: '0.18em', color: '#666' }}>
      ...
      <div className="flex flex-col md:flex-row">
        ...
        {/* ─── SIDEBAR ─── */}
        <aside className="hidden md:flex flex-col justify-between w-44 lg:w-52 flex-shrink-0 border-r sticky top-0 h-screen py-8 px-5" style={{ borderColor: '#1a1d2e' }}>
          <div>
            <div className="mb-8">
            ...
            <nav className="flex flex-col gap-0.5">
              {CATEGORIES.map((cat) => (
                ...
                >
                  {activeCategory === cat && (
                    <span style={{ position: 'absolute', left: 0, top: '50%', transform: 'translateY(-50%)', width: '2px', height: '14px', background: '#d4af37', borderRadius: '1px' }} />
                  )}
                  {cat}
                </button>
              ))}
          ...
          <div className="flex flex-col gap-0.5">
            ...
            {[{ icon: '🔍', label: 'SEARCH' }, { icon: '👤', label: 'SIGN IN' }, { icon: '📖', label: 'SUBSCRIBE' }].map(item => (
              <button key={item.label} style={{ fontSize: '9px', letterSpacing: '2px', color: '#555', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left', padding: '6px 0', fontFamily: "'Inter', sans-serif" }}>
                {item.icon} &nbsp;{item.label}
              </button>
            ))}
            <div className="flex gap-4 mt-3">
              {['IG', 'PT', 'IN'].map(s => (<span key={s} style={{ fontSize: '9px', color: '#444', cursor: 'pointer', letterSpacing: '1px' }}>{s}</span>))}
            </div>
          </div>
        </aside>
        ...
        <main className="flex-1 min-w-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[3fr_2fr] border-b" style={{ borderColor: '#1a1d2e' }}>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1.2, ease }}
              ...
              <Link href={`/journal/${hero1?.slug || '#'}`} className="block h-full">
                <img src={hero1?.cover_image || ARTICLES[0].cover_image!} alt={hero1?.title}
                ...
                <div className="absolute bottom-0 left-0 right-0 p-7 md:p-10">
                  <div className="flex items-center gap-3 mb-4">
                    <span style={{ fontSize: '9px', letterSpacing: '3px', color: '#d4af37', fontFamily: "'Inter', sans-serif" }}>{hero1?.category || 'VISION'}</span>
                    <div style={{ width: '24px', height: '1px', background: '#d4af37' }} />
                  </div>
                  <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(34px, 5vw, 68px)', fontWeight: 700, lineHeight: 1.0, color: '#ffffff', marginBottom: '14px', letterSpacing: '-0.01em' }}>
                    {hero1?.title}
                  </h2>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1.2, delay: 0.12, ease }}
              ...
              <Link href={`/journal/${hero2?.slug || '#'}`} className="block h-full">
                <img src={hero2?.cover_image || ARTICLES[1].cover_image!} alt={hero2?.title}
                  className="absolute inset-0 w-full h-full object-cover transition-transform duration-[3s] group-hover:scale-[1.04]"
                  style={{ transitionTimingFunction: 'cubic-bezier(0.16,1,0.3,1)' }} />
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 md:p-9">
                  <div className="flex items-center gap-3 mb-4">
                    <span style={{ fontSize: '9px', letterSpacing: '3px', color: '#d4af37' }}>{hero2?.category || 'CRAFT'}</span>
                    <div style={{ width: '24px', height: '1px', background: '#d4af37' }} />
                  </div>
                  <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(26px, 3.8vw, 48px)', fontWeight: 700, lineHeight: 1.05, color: '#ffffff', marginBottom: '12px', letterSpacing: '-0.01em' }}>
                    {hero2?.title}
                  </h2>
            ...
            {[card1, card2, card3].filter(Boolean).map((post, i) => (
              <motion.div key={post?.id || i}
                ...
                <Link href={`/journal/${post?.slug || '#'}`} className="block h-full">
                  <img src={post?.cover_image || ARTICLES[i + 2].cover_image!} alt={post?.title}
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4 md:p-5">
                    <div className="flex items-center gap-2 mb-2.5">
                      <span style={{ fontSize: '8px', letterSpacing: '2.5px', color: '#d4af37' }}>{post?.category}</span>
              ...
              {subscribed ? (
                ...
                <form onSubmit={(e) => { e.preventDefault(); if (email) setSubscribed(true) }} className="flex flex-col gap-2">
                  <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="Enter your email" required
                    style={{ width: '100%', padding: '10px 12px', background: 'transparent', border: '1px solid #2a2a3e', color: '#e8e8e8', fontSize: '11px', fontFamily: "'Inter', sans-serif", outline: 'none', transition: 'border-color 0.3s' }}
                    onFocus={e => (e.target.style.borderColor = '#d4af37')} onBlur={e => (e.target.style.borderColor = '#2a2a3e')} />
                  <button type="submit"
                    style={{ width: '100%', padding: '10px', background: '#d4af37', color: '#070a1a', fontSize: '9px', letterSpacing: '2px', fontWeight: 700, border: 'none', cursor: 'pointer', fontFamily: "'Inter', sans-serif" }}
          ...
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1, ease }}
            ...
            <div className="flex items-center gap-5 px-7 md:px-10 py-8 md:py-10 border-b md:border-b-0 md:border-r" style={{ borderColor: '#1a1d2e', background: 'rgba(212,175,55,0.02)' }}>
              ...
              <div>
                <blockquote style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(14px, 1.8vw, 19px)', fontStyle: 'italic', color: '#c8c8d8', lineHeight: 1.6, marginBottom: '10px', fontWeight: 400 }}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4">
              {FEATURES.map((f, i) => (
                <motion.div key={f.label}
                  ...
                  <span className="transition-transform duration-300 group-hover:scale-110" style={{ fontSize: '20px', color: '#d4af37' }}>{f.icon}</span>
                  <span style={{ fontSize: '8px', letterSpacing: '2px', color: '#d4af37', textTransform: 'uppercase', lineHeight: 1.4, textAlign: 'center' }}>{f.label}</span>
                </motion.div>
              ))}
            </div>
          </motion.div>
