...
function AuthenticateContent() {
  ...
  const lookupSerial = useCallback(async (s: string) => {
    if (!s.trim()) return
    setLoading(true); setError(''); setNotFound(false); setAsset(null); setLegacy(null); setClaimResult(null)
    const normalized = s.trim().toUpperCase()
    setSerial(normalized)

    // 1. Check sovereign_assets (new NFT system)
    ...
    if (sov) { setAsset(sov); setLoading(false); return }

    // 2. Fallback: product_authentication (legacy)
    ...
    if (auth) { setLegacy(auth); setLoading(false); return }
  ...
  const tokenId = asset?.token_id || (legacy?.nft_token_id ? parseInt(legacy.nft_token_id) : null) || claimResult?.tokenId
    ...
    <div className="min-h-screen bg-[#050505] overflow-x-hidden">
      ...
      <section className="border-b border-[#0a0a0a]">
        <div className="px-5 md:px-12 lg:px-20 py-10 md:py-14 max-w-[900px] mx-auto">
          <div className="flex gap-0 border border-[#111] mb-8 w-fit">
            {(['scan', 'enter'] as const).map(t => (
              <button key={t} onClick={() => setTab(t)}
                className={`px-6 py-3 text-[8px] tracking-[0.45em] uppercase transition-colors duration-300 ${tab === t ? 'bg-[#c9a054]/10 text-[#c9a054]' : 'text-zinc-600 hover:text-zinc-400'}`}>
                {t === 'scan' ? 'Enter Serial' : 'QR Code'}
              </button>
            ))}
          </div>
      ...
      {(asset || legacy) && !loading && (
        <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease }}
          className="px-5 md:px-12 lg:px-20 py-10 md:py-16 max-w-[1000px] mx-auto">
          ...
          <div className={`border p-4 flex items-center gap-4 mb-8 ${isMinted ? 'border-emerald-900/30 bg-[#020a02]' : 'border-amber-900/20 bg-[#0a0800]'}`}>
            ...
            <div>
              <p className={`text-[8px] tracking-[0.5em] uppercase ${isMinted ? 'text-emerald-600' : 'text-amber-700'}`}>
                {isMinted ? 'Sovereign Asset · Verified & Minted' : 'Sovereign Asset · Pending Claim'}
              </p>
              <p className={`text-xs font-light mt-0.5 ${isMinted ? 'text-emerald-900' : 'text-amber-900'}`}>
                {isMinted ? 'Digital provenance established on Polygon Mainnet' : 'This asset awaits sovereign claim. Connect wallet to mint.'}
              </p>
            </div>
          </div>
            ...
            <div>
              ...
              {isMinted && tokenId !== null && (
                <div className="flex gap-3 flex-wrap">
                  ...
                  <a href={`https://opensea.io/assets/matic/${process.env.NEXT_PUBLIC_NFT_CONTRACT_ADDRESS || '0x'}/${tokenId}`}
                  ...
                  <div className="px-5 py-4 space-y-3">
                    ...
                    {ownerWallet && (
                      <div>
                        ...
                        <p className="font-mono text-[9px] text-zinc-500 break-all">{ownerWallet}</p>
                      </div>
                    )}
                  </div>
                </div>
      ...
      <section className="border-t border-[#0a0a0a] px-5 md:px-12 lg:px-20 py-12 max-w-[1400px] mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            ...
            { icon: '◇', title: 'ERC-2981 Royalties', desc: 'Every secondary market transaction automatically routes a royalty back to The House — ensuring perpetual provenance.' },
