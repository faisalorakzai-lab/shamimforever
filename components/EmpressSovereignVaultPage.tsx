        ...
        <div id="acquire" style={{ background:'#030303', padding:'110px 40px', position:'relative', overflow:'hidden' }}>
          ...
          <div style={{ maxWidth:860, margin:'0 auto', position:'relative', zIndex:2 }}>
            ...
            {orderResult ? (
              ...
              <>
                <div className="ev-pay-grid" style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:2, marginBottom:40 }}>
                  {([['crypto','Crypto · USDT/USDC'],['pkr_manual','Bank Transfer'],['cod','COD']] as [PayMethod,string][]).map(([m,l]) => (
                    <button key={m} onClick={() => setPayMethod(m)} style={{ padding:'18px 12px', background: payMethod===m?'rgba(196,30,58,0.08)':'rgba(255,255,255,0.02)', border: payMethod===m?'1px solid ' + RUBY:'1px solid rgba(255,255,255,0.07)', color: payMethod===m?GOLD2:'rgba(255,255,255,0.35)', fontFamily:MONO, fontSize:8, letterSpacing:'0.2em', textTransform:'uppercase', cursor:'pointer', transition:'all .3s' }}>
                      {l}
                    </button>
                  ))}
                </div>
                <div style={{ marginBottom:32 }}>
                  <div style={{ fontFamily:MONO, fontSize:7, letterSpacing:'0.4em', textTransform:'uppercase', color:'rgba(201,160,84,0.32)', marginBottom:16 }}>Delivery Information</div>
                ...
                <AnimatePresence mode="wait">
                  {payMethod==='crypto' && (
                    <motion.div key="cr" initial={{ opacity:0,y:14 }} animate={{ opacity:1,y:0 }} exit={{ opacity:0,y:-14 }} transition={{ duration:.4 }}>
                      <div style={{ marginBottom:18, padding:'13px 20px', border:'1px solid rgba(201,160,84,0.09)', background:'rgba(201,160,84,0.025)' }}>
                        <div style={{ fontFamily:MONO, fontSize:8, letterSpacing:'0.2em', color:'rgba(201,160,84,0.42)', marginBottom:5, textTransform:'uppercase' }}>Polygon Network · USDT / USDC / OKBOND</div>
                        <div style={{ fontFamily:SERIF, fontSize:13, color:'rgba(255,255,255,0.32)', lineHeight:1.6 }}>10% discount with OKBOND · Blockchain confirmed on Polygon</div>
                      </div>
                      <Web3PaySection priceUsd={product.price_usd} onSuccess={handleCryptoSuccess}/>
                    </motion.div>
                  )}
                  {payMethod==='pkr_manual' && (
                    <motion.div key="pk" initial={{ opacity:0,y:14 }} animate={{ opacity:1,y:0 }} exit={{ opacity:0,y:-14 }} transition={{ duration:.4 }}>
                      <div style={{ border:'1px solid rgba(201,160,84,0.09)', padding:'32px', background:'rgba(0,0,0,0.26)', marginBottom:20 }}>
                        <div style={{ fontFamily:MONO, fontSize:7, letterSpacing:'0.4em', textTransform:'uppercase', color:'rgba(201,160,84,0.32)', marginBottom:22 }}>Bank Transfer Details</div>
                        {[
                          ...
                          <div key={lab} style={{ marginBottom:18,paddingBottom:18,borderBottom:'1px solid rgba(255,255,255,0.04)' }}>
                            <div style={{ fontFamily:MONO,fontSize:7,letterSpacing:'0.25em',color:'rgba(255,255,255,0.22)',textTransform:'uppercase',marginBottom:5 }}>{lab}</div>
                            <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center' }}>
                              <span style={{ fontFamily:SERIF,fontSize:15,color:GOLD }}>{val}</span>
                              <CopyBtn text={val}/>
                            </div>
                            <div style={{ fontFamily:MONO,fontSize:7.5,color:'rgba(255,255,255,0.2)',marginTop:3 }}>{sub}</div>
                          </div>
                        ))}
                        <div style={{ border:'2px dashed ' + (receipt?'rgba(0,255,157,0.25)':'rgba(201,160,84,0.13)'), padding:'26px', textAlign:'center', cursor:'pointer' }}
                          onClick={() => document.getElementById('receipt-empress')?.click()}>
                          <input id="receipt-empress" type="file" accept="image/*,application/pdf" style={{ display:'none' }} onChange={e => setReceipt(e.target.files?.[0]||null)}/>
                          {receipt ? (
                            <div style={{ display:'flex',alignItems:'center',justifyContent:'center',gap:10 }}>
                              <Check size={14} color="#00ff9d"/>
                              <span style={{ fontFamily:SERIF,fontSize:13,color:'rgba(0,255,157,0.72)' }}>{receipt.name}</span>
                              <button onClick={e => {e.stopPropagation();setReceipt(null)}} style={{ background:'none',border:'none',cursor:'pointer',color:'rgba(255,255,255,0.26)' }}><X size={12}/></button>
                            </div>
                          ) : (
                            <>
                              <Upload size={16} color={GOLD} style={{ marginBottom:8 }}/>
                              <div style={{ fontFamily:MONO,fontSize:8,letterSpacing:'0.25em',color:'rgba(201,160,84,0.42)',textTransform:'uppercase' }}>Upload Payment Receipt</div>
                            </>
                          )}
                        </div>
                      </div>
                      <button onClick={handleManual} disabled={submitting||!receipt}
                        style={{ width:'100%', background: receipt?'linear-gradient(135deg,rgba(196,30,58,0.16),rgba(201,160,84,0.06))':'rgba(255,255,255,0.02)', border:'1px solid ' + (receipt?RUBY:'rgba(255,255,255,0.08)'), color: receipt?GOLD2:'rgba(255,255,255,0.2)', padding:'18px', fontFamily:MONO, fontSize:8.5, letterSpacing:'0.4em', textTransform:'uppercase', cursor:receipt?'pointer':'not-allowed', transition:'all .3s' }}>
                        {submitting?'Processing Allocation...':'Submit Imperial Allocation'}
                      </button>
                    </motion.div>
                  )}
                  {payMethod==='cod' && (
                    <motion.div key="cd" initial={{ opacity:0,y:14 }} animate={{ opacity:1,y:0 }} exit={{ opacity:0,y:-14 }} transition={{ duration:.4 }}>
                      <div style={{ border:'1px solid rgba(201,160,84,0.09)', padding:'32px', background:'rgba(0,0,0,0.26)', marginBottom:20 }}>
                        <div style={{ fontFamily:MONO,fontSize:7,letterSpacing:'0.4em',textTransform:'uppercase',color:'rgba(201,160,84,0.32)',marginBottom:18 }}>Cash on Delivery</div>
                        <p style={{ fontFamily:SERIF,fontSize:15,color:'rgba(255,255,255,0.42)',lineHeight:1.8,marginBottom:18 }}>
                          Your Empress Sovereign Vault will be delivered by certified white-glove courier. Payment collected upon delivery. A 5% surcharge applies.
                        </p>
                        <div style={{ display:'flex',gap:24,flexWrap:'wrap' }}>
                          <div><div style={{ fontFamily:MONO,fontSize:7,color:'rgba(255,255,255,0.22)',textTransform:'uppercase',marginBottom:5 }}>COD Amount</div><div style={{ fontFamily:SERIF,fontSize:18,color:GOLD }}>Rs {formatPKR(Math.round(product.price_pkr*1.05))}</div></div>
                          <div><div style={{ fontFamily:MONO,fontSize:7,color:'rgba(255,255,255,0.22)',textTransform:'uppercase',marginBottom:5 }}>Delivery</div><div style={{ fontFamily:SERIF,fontSize:18,color:GOLD }}>5–7 Business Days</div></div>
                        </div>
                      </div>
                      <button onClick={handleCOD} disabled={submitting||!shipping.name||!shipping.phone||!shipping.address}
                        style={{ width:'100%', background:'linear-gradient(135deg,rgba(196,30,58,0.14),rgba(201,160,84,0.05))', border:'1px solid ' + RUBY, color:GOLD2, padding:'18px', fontFamily:MONO, fontSize:8.5, letterSpacing:'0.4em', textTransform:'uppercase', cursor:'pointer', transition:'all .3s' }}>
                        {submitting?'Reserving Allocation...':'Confirm Imperial Allocation'}
                      </button>
                    </motion.div>
                  )}
                </AnimatePresence>
                {error && (
                  <div style={{ marginTop:12, padding:'12px 16px', border:'1px solid rgba(255,80,80,0.16)', background:'rgba(255,80,80,0.04)', fontFamily:MONO, fontSize:8.5, color:'rgba(255,120,120,0.72)' }}>{error}</div>
                )}
              </>
            )}
          </div>
