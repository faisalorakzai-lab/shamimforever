...
import { Wallet, ShieldCheck, ExternalLink, Copy, Check } from 'lucide-react'
...
export default function Web3PaySection({ priceUsd, onSuccess, compact }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-0 border border-[#1a1a1a]">
        {(Object.keys(TOKEN_META) as CoinType[]).map(c => (
          <button key={c} onClick={() => { setCoin(c); setStep('idle'); setErrMsg('') }}
            className={`flex-1 py-2.5 text-[8px] tracking-[0.18em] uppercase transition-all duration-300 border-b-2 ${coin === c ? 'text-[#c9a054] border-b-[#c9a054] bg-[#c9a054]/5' : 'text-zinc-600 border-b-transparent hover:text-zinc-400'}`}>
            {c}{c === 'OKBOND' ? ' −10%' : ''}
          </button>
        ))}
      </div>
      ...
      {/* OKBOND buy link */}
      {coin === 'OKBOND' && (
        <div className="px-4 py-3 bg-[#c9a054]/5 border border-[#c9a054]/15 flex items-center justify-between">
          <div>
      ...
      <div className="p-4 bg-[#080808] border border-[#1a1a1a]">
        <div className="flex justify-between items-center mb-3">
          ...
          <div className="text-right">
            ...
            {coin === 'OKBOND' && <p className="text-[7px] tracking-[0.2em] uppercase text-[#c9a054]">10% Discount Applied</p>}
          </div>
        </div>
        <div className="flex justify-between items-center pt-3 border-t border-[#111]">
          <p className="text-[7px] tracking-[0.3em] uppercase text-zinc-700">To Wallet</p>
      ...
      {!isConnected ? (
        <div>
          ...
          <p className="text-center text-[7px] tracking-[0.3em] uppercase text-zinc-700 mt-2">MetaMask · Trust · WalletConnect · Coinbase</p>
        </div>
        ...
        <div className="space-y-2">
          ...
          {errMsg && <p className="text-red-400/80 text-[9px]">{errMsg}</p>}
