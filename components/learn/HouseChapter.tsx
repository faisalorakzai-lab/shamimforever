import type { HouseChapter as HouseChapterData } from '@/lib/the-house-content'

export default function HouseChapter({ chapter }: { chapter: HouseChapterData }) {
  return (
    <article id={chapter.slug} className="scroll-mt-28 border-t border-[#1b1814] py-12 md:py-16">
      <div className="grid gap-7 md:grid-cols-[80px_1fr]">
        ...
        <div>
          ...
          {chapter.formula && <div className="mt-8 max-w-3xl border-y border-[#2a2116] bg-[#0a0908] px-5 py-6 text-center"><p className="font-serif text-xl font-light tracking-wide text-[#c9a054] md:text-2xl">{chapter.formula}</p><p className="mt-3 text-[8px] uppercase tracking-[0.28em] text-zinc-700">Conceptual House framework · not financial or scientific advice</p></div>}
