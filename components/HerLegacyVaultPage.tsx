'use client'
  ...
  export default function HerLegacyVaultPage({ product }: { product: Product }) {
    ...
    const scentPyramid: ScentPyramidData = story.scentPyramid ?? {
      top:        'Champagne Pear Accord · White Velvet Iris Petals · Soft Crystal Bergamot',
      heart:      'Florentine Orris Butter · Imperial White Rose · Golden Mimosa Nectar',
      base:       'Pure White Ambergris Tincture · Cashmere Woods · Royal Skin Musk · Aged Sandalwood Cream',
      top_mood:   'Radiant sophistication. Refined luxury. Effortless elegance.',
      heart_mood: 'Aristocratic femininity. Museum-grade refinement. Emotional permanence.',
      base_mood:  'Expensive. Velvety. Powerfully understated. Built to survive time.',
    }
    ...
    const handleWeb3Success = useCallback(async (txHash: string, coin: CoinType) => {
      ...
      try {
        const { data: order } = await supabase.from('orders').insert([{
          ...
          notes: `Crypto TX: ${txHash} | Coin: ${coin} | Her Legacy Vault — Grand Archive Edition`,
        }]).select().single()
        if (order) {
    ...
    const nftTraits = [
      ...
      { label: 'Rarity Tier',      value: story.nft?.rarity ?? 'GRAND ARCHIVE' },
      { label: 'Archive Status',   value: 'Grand Archive — Archive IV' },
      ...
      <div className="min-h-screen bg-[#050505] pt-20">
        ...
        <div className="max-w-[1600px] mx-auto px-6 md:px-12 lg:px-20 py-8 border-b border-[#111]">
          <div className="flex items-center gap-3 text-[9px] tracking-[0.3em] uppercase text-zinc-600">
            ...
            <span>/</span>
            <span className="text-zinc-400">Her Legacy Vault</span>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-28">
            ...
            <div>
              ...
              <div className="mt-6 p-4 border border-[#c9a054]/15 bg-gradient-to-r from-[#c9a054]/5 to-transparent flex items-center gap-4">
                <span className="text-[#c9a054] text-xl">◆</span>
                <div>
                  <p className="text-[7px] tracking-[0.45em] uppercase text-[#c9a054]">Grand Archive Allocation</p>
                  <p className="text-[7px] tracking-[0.3em] uppercase text-zinc-600">
                    Archive IV · Ultra-Limited Batch · Polygon Verified
                  </p>
                </div>
              </div>
            </div>
            ...
            <div>
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                ...
                <p className="text-[8px] tracking-[0.55em] uppercase text-[#c9a054] mb-4">
                  Perfume · For Her · GRAND ARCHIVE
                </p>
                <h1 className="font-serif font-light text-4xl md:text-5xl tracking-[0.08em] text-zinc-100 leading-tight mb-1">
                  HER LEGACY VAULT
                </h1>
                <p className="font-serif italic text-zinc-500 text-xl mb-2">The Eternal Archive</p>
                <p className="text-[8px] tracking-[0.35em] uppercase text-zinc-700 mb-8">
                  {story.tagline ?? 'Some women leave memories. Legends leave dynasties.'}
                </p>
                ...
                <div className="flex border-b border-[#111] mb-8">
                  {(['story', 'specs', 'nft', 'buy'] as const).map(tab => (
                    <button key={tab} onClick={() => setActiveTab(tab)}
                      ...
                      {tab === 'story' ? 'Story' : tab === 'specs' ? 'Specs' : tab === 'nft' ? 'Digital Twin' : 'Acquire'}
                    </button>
                  ))}
                </div>
                ...
                <AnimatePresence mode="wait">
                    ...
                    <motion.div key="story" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                      ...
                      <div>
                        ...
                        <p className="font-serif italic text-zinc-400 text-sm leading-[2] border-l-2 border-[#c9a054]/25 pl-5">
                          {story.legacy_statement ?? 'Most luxury products are created to be owned. Some are created to be admired. But only a rare few are created to become family history. Her Legacy Vault is not merely a fragrance — it is the final archive of feminine sovereignty preserved in physical form. This is not a perfume bottle. This is a legacy vessel.'}
                        </p>
                      </div>
                      <div>
                        ...
                        <p className="text-zinc-500 text-xs font-light leading-[2]">
                          {story.positioning ?? 'Her Legacy Vault was inspired by those extraordinary women whose presence becomes part of a family's identity — women whose elegance survives decades, whose strength becomes tradition, whose love becomes inheritance. At the center of this composition lies Florentine Iris (Orris Butter), a raw material so precious that years of patience are required before its soul fully emerges. Fused with Pure White Ambergris Tincture, together they create a texture unlike ordinary fragrance — creamy, powdered, velvety. The scent behaves like a memory preserved inside silk.'}
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          <PyramidLayer label="THE FIRST CHAPTER" sub="Opening Layer" value={scentPyramid.top}
                            mood={scentPyramid.top_mood} width="w-2/3" delay={0} />
                          <PyramidLayer label="THE ARCHIVE HEART" sub="Core Layer" value={scentPyramid.heart}
                            mood={scentPyramid.heart_mood} width="w-4/5" delay={0.1} />
                          <PyramidLayer label="THE IMMORTAL FOUNDATION" sub="Base Layer" value={scentPyramid.base}
                      ...
                      {story.packaging ? (
                        ...
                        <div>
                          ...
                          <p className="text-zinc-500 text-xs font-light leading-[2]">
                            Her Legacy Vault is housed inside a museum-grade crystal artifact sculpted from deep ivory-white glass illuminated by internal pearl reflections. The cap is forged from solid royal gold architecture inspired by historical crown jewels preserved inside European royal vaults. Each allocation arrives inside a sovereign archive chest crafted from matte black lacquer architecture lined with hand-finished cream velvet interiors — complete with hand-authenticated serial identity, Founder Archive Certificate, NFC Authentication Seal, and Blockchain Ownership Record.
                          </p>
                        </div>
                      )}
                    </motion.div>
                  ...
                  {activeTab === 'specs' && (
                    <motion.div key="specs" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                      className="space-y-0">
                      {([
                        ['Concentration Class',  story.specs?.concentration ?? 'Extrait de Parfum'],
                        ['Volume Allocation',    story.specs?.volume ?? '100ML — 3.4 FL. OZ.'],
                        ['Longevity',            story.specs?.longevity ?? '18–36+ Hours'],
                        ['Projection',           story.specs?.projection ?? 'Elegant Sovereign Aura'],
                        ['Sillage',              story.specs?.sillage ?? 'Refined Legacy Trail'],
                        ['Batch Philosophy',     story.specs?.allocation ?? 'Grand Archive Allocation'],
                        ['Gender Profile',       story.specs?.gender ?? 'Feminine Sovereign Luxury'],
                        ['Production Method',    story.specs?.production ?? 'Ultra-Limited Atelier Craftsmanship'],
                        ['Wearing Environment',  story.specs?.wearing_environment ?? 'Ceremonial Events · Private Gatherings · Heirloom Moments'],
                        ['Craftsmanship Origin', 'Karachi Sovereign Atelier'],
                  ...
                  {activeTab === 'nft' && (
                    <motion.div key="nft" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                      className="space-y-6">
                      <p className="text-zinc-500 text-xs font-light leading-[2]">
                        {story.nft?.description ?? 'A sovereign fragrance asset authenticated by The House of Shamim Forever. Crafted around Florentine Orris and Pure White Ambergris, this digital passport certifies ownership, provenance, archive status, rarity allocation, and lifetime collector privileges. The physical vault may travel through generations. The blockchain record remains eternal.'}
                      </p>
                    ...
                    <motion.div key="buy" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                      ...
                      <div className="pt-4 border-t border-[#0d0d0d]">
                        ...
                        <p className="text-[7px] tracking-[0.25em] uppercase text-zinc-700 leading-[2]">
                          {story.cta?.primary ?? 'Acquire The Archive'} ·{' '}
                          {story.cta?.secondary ?? 'Authenticate Sovereign Passport'} ·{' '}
                          {story.cta?.tertiary ?? 'Enter The House Vault'}
                        </p>
                      </div>
                    </motion.div>
                  )}
